...
- `OSeMOSYS_Model_MUIO_Ver5.4.txt` - the same formulation as `../data/model.v.5.4.txt`
  under the name it carries in MUIO. The MUIO **project workspace** itself (the
  interface's own database) is not distributed here; it is available from the authors
  on request.
